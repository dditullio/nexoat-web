...
- Resend está migrando de "Audiences" a "Segments" — el SDK (`resend@6.22.0`) marca el campo `audienceId` como deprecado en `contacts.create`/`update` a favor de `segments: [{ id }]`. Confirmado con `curl` que **ambos apuntan al mismo recurso** (`GET /audiences` y `GET /segments` devuelven el mismo id) y que el endpoint moderno `POST /contacts` (global, sin anidar bajo `/audiences/:id/`) con `segments` funciona igual — se usa esa forma no deprecada, no `audienceId`.
...
| Archivo                                                                                              | Cambio                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `newsletter/resend-audience.service.ts` (nuevo)                                                      | Envoltorio sobre `resend.contacts` — `upsertSubscribed(email, existingContactId)` (crea con `segments: [{ id: RESEND_AUDIENCE_ID }]` si no hay id previo, o reactiva con `update({ unsubscribed: false })` si ya existe) y `markUnsubscribed(email, contactId)`. Ningún método lanza nunca — un hipo de Resend se loguea y sigue; `subscribe`/`unsubscribe` locales nunca dependen de que esto funcione. |
| `newsletter/newsletter.service.ts`                                                                   | `subscribe` gana un paso más: después del upsert local, llama a `resendAudience.upsertSubscribed` (se espera, no fire-and-forget — hace falta el id que devuelve para persistirlo) y guarda el `resendContactId` si cambió. Nuevo método `unsubscribe(dto)`: marca `isActive: false`, `unsubscribedAt: now()` localmente, y llama a `markUnsubscribed`.                                                  |
...
1. Test de backend: `subscribe` con un contacto nuevo → llama a `contacts.create` con `segments`, guarda el `resendContactId` devuelto. ✅
2. Test de backend: `subscribe` de alguien que ya tenía `resendContactId` → llama a `contacts.update` (no `create`), reactiva. ✅
...
5. Manual: `POST /newsletter/subscribe` real → apareció en `GET /audiences/:id/contacts` de Resend con el mismo email y el `resendContactId` devuelto coincidió con el guardado en la DB. ✅
