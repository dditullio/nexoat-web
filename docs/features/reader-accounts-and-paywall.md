...
5. Manual (navegador): artículo con `scope: suscriptores_nivel_1` visto sin sesión → chip "Registrados", **aviso de "Vista previa" en la cabecera** (antes de empezar a leer, con CTA "Registrate gratis" o badge "Próximamente" según el nivel — agregado después de la primera implementación, para que el lector sepa de antemano que no es el artículo completo), contenido cortado en el marcador, bloque "Seguí leyendo gratis" al pie del contenido con botón a `/registrarme`. ✅
