...
- [`features/email-provider-resend.md`](features/email-provider-resend.md) — Resend como proveedor de email (transaccional + newsletter), cerrando el hueco de `MailService` no-op desde el primer día. 4 fases: `MailService` real + bienvenida, verificación/reset de contraseña, sync de `NewsletterSubscriber` con Resend, y la pantalla "Preferencias de correo" del menú de usuario. **Estado: fase 1 (MailService real + email de bienvenida) implementada. Fases 2-4 pendientes.**
